...
const Inscription = () => {
  useEffect(() => {
    ...
    (function () {
      ...
      document.getElementById('formulaireInscription').addEventListener('submit', async function (e) {
        e.preventDefault();
        ...
        try {
          var reponse = await fetch('/api/auth/register', {
            method: 'POST',
    ...
    <>
      ...
      <section className="stage">
        <div className="panel">
          ...
          <p className="sub">Deux minutes, et votre siège est réservé.</p>
    
          <form id="formulaireInscription">
            <label htmlFor="pseudo">Pseudo</label>
            <input type="text" id="pseudo" required minLength="3" maxLength="30"
                   autoComplete="username" placeholder="Celui que la table retiendra" />
    
            <label htmlFor="email">Email</label>
            <input type="email" id="email" required autoComplete="email" placeholder="vous@exemple.com" />
    
            <label htmlFor="password">Mot de passe</label>
            <input type="password" id="password" required minLength="8"
                   autoComplete="new-password" placeholder="Majuscule, minuscule, chiffre" />
    
            <label htmlFor="date_naissance">Date de naissance</label>
            <input type="date" id="date_naissance" required />
    
            <button type="submit" className="btn btn-gold" id="btnSubmit">S'inscrire</button>
          </form>
      ...
      <div id="victoire" aria-live="polite">
        ...
        <div className="vmsg">
          <div className="grosjeton" aria-hidden="true"><span>OK</span></div>
