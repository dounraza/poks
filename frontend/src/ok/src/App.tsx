...
// 9 seats distributed along an elliptical perimeter
const SEAT_ANGLES = [270, 310, 350, 30, 70, 110, 150, 190, 230]

// Radii for info panels (% of container half-dimensions) - Adjusted for oval
const RX_INFO = 42
const RY_INFO = 28
// Radii for chairs — further from center - Adjusted for oval
const RX_CHAIR = 52
const RY_CHAIR = 38
...
function PlayerSlot({ player, angle }: { player: Player; angle: number }) {
  ...
  return (
    <>
      ...
      {!isEmpty && (
        <div
          className={`absolute flex gap-0.5 ${isFolded ? "opacity-20" : ""}`}
          style={{
            left: infoPos.left,
            top: infoPos.top,
    ...
    <div className="room-bg w-full h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative" style={{ width: "min(90vw, 1100px)", height: "min(60vh, 500px)" }}>
        ...
        {SEAT_ANGLES.map((angle, i) => {
          ...
          // Position relative to oval
          const pos = posAt(midAngle, 50, 36) 
          return (
            <div
              key={i}
              className="absolute w-5 h-5 rounded-full bg-amber-500 blur-[3px] pointer-events-none animate-pulse"
              style={{
                left: pos.left,
                top: pos.top,
                transform: "translate(-50%, -50%)",
                boxShadow: "0 0 15px 5px rgba(251, 191, 36, 0.8)",
              }}
            />
          )
        })}
        ...
        <div className="table-felt absolute rounded-[50%]" style={{ inset: "6%" }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <div className="flex flex-row items-center gap-3 animate-pulse">
              <div className="w-9 h-9 bg-gradient-to-br from-yellow-500 to-amber-700 rounded-full flex items-center justify-center font-bold text-white text-lg border-2 border-yellow-500/60 shadow-[0_0_15px_rgba(234,179,8,0.5)]">
                A
              </div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "1.3rem", color: "#fcd34d", letterSpacing: "0.2em", textShadow: "0 0 10px rgba(252, 211, 77, 0.5)" }}>
                Afripoks
              </div>
            </div>
          </div>
